...
class BaseRGBModel(ABCModel):
    ...
    def get_sam_optimizer(self, opt_args):
        base_optimizer = torch.optim.AdamW
        optimizer = SAM(self._get_params(), base_optimizer, **opt_args)
        return optimizer, torch.cuda.amp.GradScaler() if self.device == 'cuda' else None
...
def sam_step(optimizer, scaler, loss, lr_scheduler=None, backward_only=False):
    loss1 = loss
    loss.backward()
    if not backward_only:
